    ...
    switch(exp -> op) {
        case NO_OP: {
            if(exp -> datatype == VAR_DT) {
                ...
                //printf("Z \n");
                if(!E) {
                    fprintf(stderr, "Error: Identifier %s not defined \n", exp -> u.literal);
                    exit(1);
                }
    ...
    switch(stmt -> stmt_type) {
            ...
            switch(type) {
                case BOOL_DT: case INTEGER: case FLOATING: case STRING_DT: {
                    if(sym_found(S, dec -> ident, 0)) {
                        fprintf(stderr, "Error: Redefinition of Identifier %s that already exists\n", dec -> ident);
                        exit(1);
                    }
                    //printf("I'm here \n");
                    ...
                    if(dec -> exp != NULL) {
                        if(E -> datatype == FLOATING  && (dec -> exp -> datatype == INTEGER || dec -> exp -> datatype == FLOATING)) {
                            ...
                            fprintf(stderr, "Error: Assignment error: Conflicting types \n");
                            exit(1);
                        }
                    }
                }
        ...
        case WHILE_ST: {
            ...
            if(stmt -> body_of_stmt.loop.exp -> datatype != BOOL_DT) {
                fprintf(stderr, "Error: Invalid expreesion as a loop condition --> should be of type bool \n");
                exit(1);
            }
